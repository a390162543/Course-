    ...
    series : [
      {
        ...
        itemStyle: {
          normal: {
            color: (obj)=> {
              let val = obj.data[1];
              if(val <= 1){return '#e4e5be';}
              if(val <= 4){return '#a7b6aa';}
              if(val <= 7){return '#698896';}
              if(val <= 9){return '#305d82';}
              return '#023a73';
            }
          }
        }
...
createTeacherIncomeOption = (data) => {
  // data = [{name:'t1',total:'1100',average:'500'},];
  data = data.sort((o1,o2)=>(o1.total-o2.total));
  names = [];
  data.forEach((obj)=>{
    names.push(obj.name);
    obj.value = obj.total-obj.quit;
  });
  option = {
    backgroundColor: '#404a59',
    tooltip: {},
    ...
    series: [{
      ...
      data: data.map((obj)=>(obj.total*1-obj.quit*1)),
